...
public class PlaneData
{
    ...
    public enum ViewMode { MAQUETTE, REAL };
    ...
    public float session_time_received; // XXX Time.Totalf when this data point was created
    public int update_timestamp;        // Update timestamp (seconds since UNIX epoch) associated with data update from opensky
    public UpdateState updateState;

    public float last_lat, last_lon;
    public float last_heading;          // [0,360[ degrees
    public float last_height;           // meters
    public float last_speed;            // m/s
    public float last_vertical_rate;    // m/s

    public float last_x, last_y, last_z;        // Map-centric location (units = km, origin = map origin)
    ...
    public void Recenter(ViewMode viewMode)
    {
        // Compute last known point, relative to map center (units = kilometers)
        // XXX should be dependent on view mode

        if (viewMode == ViewMode.MAQUETTE)
        {
            ...
            last_z = this.last_height / 1000.0f;

            computed_position = new Vec3(last_x, last_y, last_z);
        }
        /*else if (viewMode == ViewMode.REAL)
        {
    ...
    public void ProcessDataUpdate(float session_time_received, JSONNode node)
    {
        ...
        last_height = node[13];        // geometric altitude (meters)
        ...
        // XXX any point to interpolate this? we don't have angular (heading) speed
    ...
    public void Update(float session_time)
    {
        ...
        //double current_epoch_time = DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalSeconds;
